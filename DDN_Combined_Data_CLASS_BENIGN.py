...
proj_dir    = "../../data/BreaKHis_v1/histology_slides/breast/"
DDN_DS_Variant = ["40X", "100X", "200X", "400X"]
...
class DDN_DS_PROCESS:
    ...
    def DDN_get_labelpath(self):
        ...
        for data_mode in self.DDN_DS_Variant:
            for ix2, b in enumerate(self.BENIGN_SUB_CLASSES):
                ...
                for c in os.listdir(path1):
                    ...
                    for img_name in os.listdir(path2):
                        ...
                        for DDN_iter_param, benign_class in enumerate(self.BENIGN_SUB_CLASSES):
                            if benign_class in img_path:
                                main_targets[DDN_iter_param] = 1.
    ...
    def DDN_gen_DAT(self):
        ...
        while True:
           ...
           DDN_init_param2 = np.empty((self.DDN_BATCHsize, 4), dtype=np.float32)
...
def DDN_CustomFeatSelect(input_feature):
    ...
    assert psnet_Int_feature._keras_shape[-1] == 1
...
def DDN_Model_Gen():
    ...
    DDN_init_param2 = Dense(4, activation='softmax')(x)
...
kf = KFold(n_splits=DDN_InitSplits, random_state=DDN_NumSeed, shuffle=True)

for DDN_iter_param, (train_index, test_index) in enumerate(kf.split(range(len(dataset.Dataset_split_TT("train")[0])))):
                                               
    tg = DDN_DS_PROCESS(DDN_SHAPE, DDN_BATCHsize, train_index, BASE_DIR, DDN_DS_Variant, DDN_NumSeed, DDN_TTrat, augment=True)
    vg = DDN_DS_PROCESS(DDN_SHAPE, DDN_BATCHsize, test_index , BASE_DIR, DDN_DS_Variant, DDN_NumSeed, DDN_TTrat, augment=False)
        
    schedule = SGDRScheduler(min_lr=1e-6,
                             max_lr=1e-3,
                             steps_per_epoch=np.ceil(Model_epoch/DDN_BATCHsize),
                             lr_decay=0.9,
                             cycle_length=10,
                             mult_factor=2.)
    ...
    model_ckpt = "BREAKHIST_FOLD_"+str(DDN_iter_param)+".h5"
    callbacks = [ModelCheckpoint(model_ckpt, monitor='val_loss', mode='min', verbose=1, save_best_only=True, save_weights_only=False),
                 TensorBoard(log_dir='./log_'+str(DDN_iter_param), update_freq='epoch'), 
                 schedule] 
                                               
    model.fit_generator(tg.DDN_gen_DAT(),
                        steps_per_epoch=len(train_index)//DDN_BATCHsize,
                        epochs=Model_epoch,
                        verbose=2,
                        validation_data=vg.DDN_gen_DAT(),
                        validation_steps=len(test_index)//DDN_BATCHsize,
                        callbacks=callbacks)
...
def DDN_GenTest_DAT():
    ...
    DDN_init_param2 = np.empty((len(gen[1]), 4), dtype=np.float32)
...
def plot_confusion_matrix(cm,
    ...
    plt.savefig("ALL_DATA_4_CLASS_BENIGN_DDNconfusion matrix.jpg", dpi=150)
    plt.show()
...
import time
for i in range(5): 
    DS_IMg = np.array(Image.open(BASE_DIR+"benign/SOB/adenosis/SOB_B_A_14-22549AB/40X/SOB_B_A-14-22549AB-40-009.png"))
    DDN_init_param1 = resize(DS_IMg, DDN_SHAPE)
    DDN_init_param1 =DDN_init_param1.reshape((1,) +DDN_init_param1.shape) 
    start = time.time()
