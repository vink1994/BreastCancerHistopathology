                    ...
                    for ix2, b in enumerate(self.BENIGN_SUB_CLASSES):
                        ...
                        for c in os.listdir(path1):
                            ...
                            for img_name in os.listdir(path2):
                                ...
                                img_path = path3 
                                main_targets = np.zeros((2), dtype=np.float32) # Dual classification (benign or malignant)
                                main_targets[ix1] = 1.
...
models = []
for i in range(5):
    model = load_model("E:/PHD/shwetha_hyst_code/V6/trained_weights/Combined_Data_Mag.h5".format(i), custom_objects={'f1': f1, 'precision': precision, 'recall': recall})
